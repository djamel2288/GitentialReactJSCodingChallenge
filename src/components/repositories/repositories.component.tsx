...
export interface RepositoriesComponentProps extends RepositoriesComponentPropsFromRedux {
}
...
function RepositoriesComponent({
    ...
    const [isSearchResultsOpen, setSearchResultsIsOpen] = useState<boolean>(false)
    ...
    const [isRepoRequestInProgress, setIsRepoRequestInProgress] = useState<boolean>(false)
    ...
    const addSelectedRepo = () => {
        const selectedRepo: RepoSearchResponse | undefined = searchResults.find(r => r.selected)
        if (selectedRepo) {
            ...
            <div className={ style.add_repo_container }>
                <ClickAwayListener onClickAway={ () => setSearchResultsIsOpen(false) }>
                    <div className={ style.add_repo_input }>
                        <input
                            type="text"
                            value={ searchResults.find(r => r.selected)?.full_name ?? newRepoUrl }
                            onChange={ handleNewRepoNameChange }
                            onFocus={ openSearchResults }
                        />
                        {
                            isSearchInProgress &&
                            <span className={ style.input_loader } />
                        }
                        {
...
export type RepositoriesComponentPropsFromRedux = ConnectedProps<typeof connector>
