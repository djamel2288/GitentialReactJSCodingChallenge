...
const selectRepositoriesState = (state: any): RepositoriesDataState => state.repositories
...
export const isRepoListEmptySelector = createSelector(
    [selectRepositoriesState],
    (reposState: RepositoriesDataState): boolean =>
        isEmpty(reposState.repoList)
)

export const selectedRepoUrlSelector = createSelector(
    [selectRepositoriesState],
    (reposState: RepositoriesDataState): string =>
        reposState.selectedRepoUrl
)

export const selectedRepoFullNameSelector = createSelector(
    [selectRepositoriesState],
    (reposState: RepositoriesDataState): string =>
        reposState.repoList.find(r => r.repoUrl === reposState.selectedRepoUrl)?.repoFullName ?? ''
)
